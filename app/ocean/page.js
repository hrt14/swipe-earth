...
function Creature({ type }) {
  return <div className={`creature-art creature-${type}`} aria-hidden="true"><i/><b/><em/><span/><small/></div>;
}
    ...
    <main className="ocean-experience">
      ...
      <aside className="depth-rail" aria-hidden="true">
        ...
        {[0, 200, 1000, 4000, 6000, 11000].map((value) => (
          <div className="rail-mark" key={value} style={{ top: `${(value / 11000) * 100}%` }}><span>{value.toLocaleString()}m</span></div>
        ))}
      </aside>

      <section className="surface-scene">
        <div className="sun-disc" />
        <div className="sun-ray ray-a" /><div className="sun-ray ray-b" />
        <div className="wave wave-a" /><div className="wave wave-b" />
        <div className="surface-copy">
          <p>SWIPE EARTH: OCEAN</p>
          <h1>海面から、<br />地球の底へ。</h1>
          <span>SCROLL / SWIPE TO DIVE</span><b>↓</b>
        </div>
      </section>

      <div className="water-column">
        <div className="particles" aria-hidden="true">{Array.from({ length: 26 }).map((_, i) => <i key={i} />)}</div>
        {zones.map((z) => <div className="zone-divider" key={z.label} style={{ top: `${(z.from / 11000) * 100}%` }}><span>{z.label}</span><small>{z.jp}</small></div>)}
        {encounters.map((item, index) => (
          <article className={`encounter-row ${index % 2 ? 'right' : 'left'} ${item.mystery ? 'mystery' : ''} ${item.scale === 'giant' ? 'giant' : ''}`} key={item.depth} style={{ top: `${3 + (item.depth / 11000) * 92}%` }}>
            <div className="encounter-depth">{item.depth.toLocaleString()} m</div>
            <div className="creature-stage"><Creature type={item.type} /></div>
            <div className="encounter-copy">
              <p>{item.mystery ? 'UNCONFIRMED / LEGEND' : 'ENCOUNTER'}</p>
              <h2>{item.name}</h2><h3>{item.jp}</h3><div className="rule" /><div className="note">{item.note}</div>
            </div>
          </article>
        ))}
      </div>

      <section className="ocean-floor"><div className="floor-glow" /><p>10,900 m — CHALLENGER DEEP</p><h2>ここまで来ても、<br />まだ未知は残っている。</h2><a href="/">EXPLORE ANOTHER WORLD →</a></section>
    </main>
  );
}
