...
const encounters = [
  { depth: 15, name: 'SEA TURTLE', jp: 'アオウミガメ', type: 'turtle', sheet: 'real', col: 0, row: 0, cols: 3, rows: 2, note: 'まだ太陽の光が強い。海面近くでは、呼吸のために水面へ上がるウミガメに出会う。' },
  { depth: 60, name: 'MANTA RAY', jp: 'オニイトマキエイ', type: 'manta', sheet: 'real', col: 1, row: 0, cols: 3, rows: 2, note: '大きな翼のような胸びれで、青い水の中をゆっくり滑空する。' },
  { depth: 120, name: 'BLUE WHALE', jp: 'シロナガスクジラ', type: 'whale', sheet: 'real', col: 2, row: 0, cols: 3, rows: 2, giant: true, note: '地球上で最大の動物。体長は30m近くに達することもある。' },
  { depth: 250, name: 'GIANT SQUID', jp: 'ダイオウイカ', type: 'squid', sheet: 'real', col: 0, row: 1, cols: 3, rows: 2, note: '光が急速に弱くなる薄明帯。巨大な眼で、わずかな光を捉える。' },
  { depth: 600, name: 'ANGLERFISH', jp: 'チョウチンアンコウ', type: 'angler', sheet: 'real', col: 1, row: 1, cols: 3, rows: 2, note: '太陽光はほぼ届かない。自ら発する光が、この世界の目印になる。' },
  { depth: 1000, name: 'VAMPIRE SQUID', jp: 'コウモリダコ', type: 'vampire', sheet: 'real', col: 2, row: 1, cols: 3, rows: 2, note: '酸素の少ない暗黒の海に適応した、不思議な深海生物。' },
  { depth: 2200, name: 'GIANT ISOPOD', jp: 'ダイオウグソクムシ', type: 'isopod', sheet: 'real', col: 2, row: 1, cols: 3, rows: 2, note: '水圧は地上とは比較にならない。海底を歩く巨大な等脚類。' },
  { depth: 4000, name: 'HITOGATA', jp: 'ヒトガタ', type: 'hitogata', sheet: 'legend', col: 0, row: 0, cols: 2, rows: 2, mystery: true, note: '南極海などで語られる白い人型の未確認存在。ここから先は、事実と伝承の境界が曖昧になる。' },
  { depth: 6000, name: 'KRAKEN', jp: 'クラーケン', type: 'kraken', sheet: 'legend', col: 1, row: 0, cols: 2, rows: 2, mystery: true, giant: true, note: '巨大な触手で船を沈めると語られてきた伝説の怪物。もちろん、確認された生物ではない。' },
  { depth: 8200, name: 'HADAL SHADOW', jp: '超深海の影', type: 'shadow', sheet: 'legend', col: 0, row: 1, cols: 2, rows: 2, mystery: true, note: '観測できる範囲は急激に狭くなる。暗闇の向こうに何がいるかは、まだ分からない。' },
  { depth: 10900, name: 'THE UNKNOWN', jp: '未知', type: 'unknown', sheet: 'legend', col: 1, row: 1, cols: 2, rows: 2, mystery: true, note: '地球の最深部。人類はここまで来た。それでも、海のすべてを知ったわけではない。' }
];
...
function CreatureImage({ item }) {
  const src = item.sheet === 'real' ? '/ocean/creatures.webp' : '/ocean/legends.webp';
  return (
    <div className={`sprite-frame sprite-${item.type}`}>
      <img
        src={src}
        alt={item.jp}
        style={{
          width: `${item.cols * 100}%`,
          height: `${item.rows * 100}%`,
          left: `${-item.col * 100}%`,
          top: `${-item.row * 100}%`
        }}
      />
    ...
    <main className="ocean-v3">
      ...
      <section className="journey">
        ...
        {encounters.map((item,index) => <article key={item.type} className={`encounter ${index%2?'right':''} ${item.mystery?'mystery':''} ${item.giant?'giant':''} ${active.type===item.type?'active':''}`} style={{top:`${2.8+(item.depth/11000)*94}%`}}>
          <div className="visual"><CreatureImage item={item} /></div>
